...
@section('content')
    <header class="mb-10">
        ...
        <p class="text-slate-500 font-medium">Perbarui informasi event <span
                class="text-indigo-600">"{{ $event->title }}"</span></p>
    </header>
        ...
        <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="grid grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="category_id" class="w-full px-5 py-3 rounded-xl border border-slate-200 outline-none"
                        required>
                ...
                <div class="col-span-2">
                    ...
                    <div
                        class="flex items-start gap-6 p-4 border border-dashed border-slate-200 rounded-2xl bg-slate-50/50">
                        <div class="shrink-0 text-center">
                            <p class="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-2">Poster Saat Ini
                            </p>
                            <img src="{{ asset('storage/' . $event->poster_path) }}"
                                class="w-24 h-32 rounded-xl object-cover shadow-md border-2 border-white">
                        </div>
                        <div class="flex-1">
                            <p class="text-xs text-slate-500 mb-3 leading-relaxed">Pilih file baru jika ingin mengganti
                                poster. Kosongkan jika tidak ingin mengubah gambar.</p>
            ...
            <div class="flex justify-end gap-4 mt-10 pt-6 border-t border-slate-100">
                ...
                <button type="submit"
                    class="px-10 py-3 bg-indigo-600 text-white rounded-2xl font-bold shadow-lg shadow-indigo-100 hover:bg-indigo-700 transform active:scale-95 transition">
                    Simpan Perubahan
                </button>
            </div>
        </form>
